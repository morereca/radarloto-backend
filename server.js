...
async function syncHistory(force = false) {
  ...
  syncPromise = (async () => {
    ...
    try {
      await importHistory("primitiva", startYear, year);
    } catch (err) {
      logSync(`Error al importar primitiva: ${err.message}`);
    }

    try {
      await importHistory("euromillones", startYear, year);
    } catch (err) {
      logSync(`Error al importar euromillones: ${err.message}`);
    }
