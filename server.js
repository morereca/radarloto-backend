    ...
    for (const item of generatedRows) {
      ...
      const createdDate = String(createdAt).slice(0, 10);
        ...
        const futureRows = (drawsByGame[game] || []).filter((row) => {
          ...
          // same day check
          const sameDay = drawDate.toDateString() === created.toDateString();

          if (sameDay) {
            const createdHours = created.getHours();
            const createdMinutes = created.getMinutes();

            // before cutoff -> count this draw
            if (
          ...
          // future dates always count
          return drawDate > created;
        });
...
// DEBUG: listar combinaciones generadas
app.get("/api/debug/generated", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT id, game, numbers_json, reintegro, created_at
      FROM es_generated
      ORDER BY created_at DESC
      LIMIT 100
    `);
    ...
    console.error("DEBUG GENERATED ERROR:", e);
