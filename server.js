...
app.get("/api/prize-ranking", async (req, res) => {
  try {
    ...
    const ranking = generatedRanking.map(item => {
      ...
      const genReintegro = item.reintegro != null ? Number(item.reintegro) : null;
      ...
      for (const draw of draws) {
        if (draw.drawDate <= createdAt) continue;
        ...
        if (game === "primitiva") {
          ...
          const category = classifyPrimitiva(numMatches, false, reintegroHit);

          if (category) {
            totalHits++;
            if (numMatches >= 4) realPrizes++;

            if (
              !bestResult ||
              numMatches > bestResult.matches_main ||
              (numMatches === bestResult.matches_main && (reintegroHit ? 1 : 0) > (bestResult.matches_extra || 0))
            ) {
              bestResult = {
                label: category,
          ...
          if (category) {
            ...
            if (
              !bestResult ||
              numMatches > bestResult.matches_main ||
              (numMatches === bestResult.matches_main && starMatches > (bestResult.matches_extra || 0))
            ) {
              bestResult = {
                label: category,
                matches_main: numMatches,
      ...
      return {
        game,
        outcome_label: bestResult?.label || "Sin premio relevante",
    ...
    res.json({
      ...
      methodology: "Solo sorteos posteriores a la generación"
    });
...
app.post("/api/admin/clear-primitiva", async (req, res) => {
  try {
    const result = db.prepare("DELETE FROM draws WHERE game = 'primitiva'").run();
    res.json({ ok: true, deleted: result.changes });
  } catch (error) {
    console.error("clear-primitiva error", error);
    res.status(500).json({ ok: false, error: "No se pudo borrar Primitiva" });
  }
});
