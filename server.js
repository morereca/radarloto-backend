...
function simulateHistorical(game, generated) {
  const draws = historyState[game] || [];
  const genNums = new Set((generated.numbers || []).map(Number));
  const genStars = new Set((generated.stars || []).map(Number));
  const genReintegro = generated.reintegro != null ? Number(generated.reintegro) : null;
  ...
  function considerBest(label, matchesMain, matchesExtra, drawDate) {
    if (
      !bestResult ||
      matchesMain > bestResult.matches_main ||
      (matchesMain === bestResult.matches_main && matchesExtra > bestResult.matches_extra)
    ) {
      bestResult = {
        label,
  ...
  for (const draw of draws) {
    ...
    if (game === "primitiva") {
      const complementaryHit = draw.complementary != null && genNums.has(Number(draw.complementary));
      const reintegroHit =
        genReintegro !== null &&
        draw.reintegro != null &&
        Number(draw.reintegro) === genReintegro;
      ...
      if (category) {
        ...
        considerBest(category, numMatches, reintegroHit ? 1 : 0, draw.drawDate);
      ...
      if (category) {
        ...
        if (numMatches >= 4 || (numMatches === 3 && starMatches >= 2)) {
          realPrizes += 1;
        }

        considerBest(category, numMatches, starMatches, draw.drawDate);
  ...
  return {
    ...
    winning_draws: realPrizes,
    best_result: bestResult,
    breakdown,
    latest_hits: sampleHits.slice(0, 5),
...
function buildAnalysis(game, generated) {
  ...
  return {
    ...
    reasons: [
      ...
      `Simulación histórica (${historicalSimulation.window_years} años): ${historicalSimulation.real_prizes} premios relevantes`
    ]
  };
}
...
function registerStats(game, historicalSimulation = null) {
  ...
  if ((historicalSimulation?.real_prizes || 0) > 0) stats.won_count += 1;
}
...
app.post("/api/simulate-combination", async (req, res) => {
  try {
    ...
    const numbers = Array.isArray(req.body?.numbers) ? req.body.numbers.map(Number) : [];
    const stars = Array.isArray(req.body?.stars) ? req.body.stars.map(Number) : [];
    const reintegro = req.body?.reintegro != null ? Number(req.body.reintegro) : null;

    if (game === "primitiva" && numbers.length !== 6) {
      return res.status(400).json({ error: "La Primitiva requiere 6 números" });
    }

    if (game === "euromillones" && (numbers.length !== 5 || stars.length !== 2)) {
      return res.status(400).json({ error: "Euromillones requiere 5 números y 2 estrellas" });
    }
    ...
    const generated = {
      ...
      stars: game === "euromillones" ? [...stars].sort((a, b) => a - b) : [],
      reintegro: game === "primitiva" ? reintegro : null
    };
    ...
    res.status(500).json({ error: "Error interno al simular la combinación" });
  }
});
