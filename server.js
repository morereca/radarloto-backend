...
import { fetchOfficialNumberStats, refreshOfficialNumberStatsCache, refreshAllOfficialNumberStatsCache, getCachedOfficialNumberStats } from './services.officialStats.js';
...
app.get('/api/number-stats', async (req, res) => {
  ...
  try {
    ...
    const response = {
      ...
      source: stats.source || 'draws'
    };

    if (Number(stats.coverage?.total || 0) < 50) {
      const fresh = await refreshOfficialNumberStatsCache(game);
      response.coverage = fresh.coverage;
      response.hot_numbers = fresh.hotMain.slice(0, 12);
      response.cold_numbers = fresh.coldMain.slice(0, 12);
      response.hot_extra = fresh.hotExtra.slice(0, 6);
      response.cold_extra = fresh.coldExtra.slice(0, 6);
      response.main_counts = fresh.mainCounts;
      response.extra_counts = fresh.extraCounts;
      response.source = 'official-selae';
    }
...
app.get('/api/official-number-stats', async (req, res) => {
  const game = req.query.game;
  if (!game || !['primitiva', 'euromillones'].includes(game)) {
    return res.status(400).json({ error: 'game requerido' });
  }
  try {
    const stats = await fetchOfficialNumberStats(game);
    res.json({
      game,
      coverage: stats.coverage,
      hot_numbers: stats.hotMain.slice(0, 12),
...
app.post('/api/admin/refresh-official-number-stats', async (_req, res) => {
  try {
    const result = await refreshAllOfficialNumberStatsCache();
    res.json({ ok: true, result });
  } catch (error) {
    res.status(500).json({ error: String(error.message || error) });
  }
...
app.post('/api/admin/import-history', async (req, res) => {
  try {
    ...
    const defaults = {
      primitiva: { start: 1987, end: year },
      euromillones: { start: 2004, end: year }
    };
...
app.post('/api/admin/import-history-all', async (_req, res) => {
  try {
    const year = new Date().getFullYear();
    const primitiva = await importHistory('primitiva', 1987, year);
    const euromillones = await importHistory('euromillones', 2004, year);
    res.json({
      ok: true,
