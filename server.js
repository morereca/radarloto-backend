...
app.get("/api/prize-ranking", async (_req, res) => {
  try {
    await syncHistory(false);
    ...
    for (const item of generatedRanking) {
      ...
      for (const row of rows) {
        ...
        if (game === "primitiva") {
          ...
          if (category) {
            totalHits++;
            if (numMatches >= 4 || (numMatches === 3 && starMatches >= 2)) realPrizes++;
            if (
              !bestResult ||
              numMatches > bestResult.matches_main ||
    ...
    res.json({
      ...
      methodology: "Solo sorteos futuros tras la generación"
    });
  } catch (error) {
