...
import { importLocalHistory, importAllLocalHistory, inspectLocalHistoryFiles } from './services.historyLocal.js';
...
app.get('/api/history-local-files', (_req, res) => {
  try {
    res.json({ ok: true, files: inspectLocalHistoryFiles() });
  } catch (error) {
    res.status(500).json({ error: String(error.message || error) });
  }
});

app.post('/api/admin/import-history-local', (req, res) => {
  try {
    const game = req.body.game;
    if (game) {
      const result = importLocalHistory(game);
      return res.json({ ok: true, result });
    }
    const result = importAllLocalHistory();
    return res.json({ ok: true, result });
  } catch (error) {
    res.status(500).json({ error: String(error.message || error) });
  }
