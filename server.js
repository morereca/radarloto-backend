  ...
  try {
    ...
    for (const item of generatedRows) {
      ...
      const futureRows = (drawsByGame[game] || []).filter((row) => {
        ...
        const sameDay = drawDateStr === createdDateStr;

        if (sameDay) {
          const createdHours = created.getUTCHours();
          const createdMinutes = created.getUTCMinutes();
        ...
        return drawDateStr > createdDateStr;
      });
