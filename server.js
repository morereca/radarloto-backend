...
app.get("/api/prize-ranking", async (req, res) => {
  try {
    ...
    const ranking = generatedRanking.map(item => {
      ...
      const genNums = new Set((item.generated?.numbers || []).map(Number));
      const genStars = new Set((item.generated?.stars || []).map(Number));
      const genReintegro = item.generated?.reintegro != null ? Number(item.generated.reintegro) : null;
      ...
      for (const draw of draws) {
        ...
        if (game === "primitiva") {
          ...
          if (category) {
            ...
            if (!bestResult || numMatches > bestResult.matches_main) {
              bestResult = {
                label: category,
