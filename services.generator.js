...
function safePrimitivaReintegro(stats, mode) {
  const fallback = rand(0, 9);

  if (!stats) return fallback;

  if (mode === 'Números calientes') {
    return safeNumber(stats.hotExtra?.[0], fallback);
  }

  if (mode === 'Números fríos') {
    return safeNumber(stats.coldExtra?.[0], fallback);
  }

  if (mode === 'Radar Loto IA') {
    return safeNumber(stats.hotExtra?.[0], safeNumber(stats.coldExtra?.[0], fallback));
  }
...
export function generatePrimitiva(mode) {
  ...
  const stats = getNumberStats('primitiva');
  const reintegro = safePrimitivaReintegro(stats, mode);
