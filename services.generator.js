...
function pickRandomFromTop(ranked, count, topSize, fallbackMin, fallbackMax) {
  const source = [...new Set((ranked || []).map(Number).filter((n) => !Number.isNaN(n)))].slice(0, topSize);
  const out = [];

  for (const n of shuffle(source)) {
    if (!out.includes(n)) out.push(n);
...
function pickRandomOneFromTop(ranked, topSize, fallbackMin, fallbackMax) {
  const source = [...new Set((ranked || []).map(Number).filter((n) => !Number.isNaN(n)))].slice(0, topSize);
  if (source.length === 0) return rand(fallbackMin, fallbackMax);
  return source[rand(0, source.length - 1)];
}

function pickIaMain(game, count) {
  const stats = getNumberStats(game);
  const hot = stats.hotMain.slice(0, Math.max(count + 6, 12));
  const cold = stats.coldMain.slice(0, Math.max(count + 6, 12));
  const merged = [];
  ...
  return pickRandomFromTop(merged, count, Math.max(count + 6, 12), 1, game === 'primitiva' ? 49 : 50);
}

function pickIaExtra(game, count) {
  const stats = getNumberStats(game);
  const hot = stats.hotExtra.slice(0, Math.max(count + 3, 6));
  const cold = stats.coldExtra.slice(0, Math.max(count + 3, 6));
  const merged = [];
...
function pickSmartPrimitivaReintegro(mode) {
  ...
  if (mode === 'Números calientes') {
    return safeNumber(pickRandomOneFromTop(stats.hotExtra, 5, 0, 9), fallback);
  }

  if (mode === 'Números fríos') {
    return safeNumber(pickRandomOneFromTop(stats.coldExtra, 5, 0, 9), fallback);
  }

  if (mode === 'Radar Loto IA') {
    const merged = [
      ...(stats.hotExtra || []).slice(0, 5),
      ...(stats.coldExtra || []).slice(0, 5)
    ];
    return safeNumber(pickRandomOneFromTop(merged, 8, 0, 9), fallback);
  }
