...
export function generatePrimitiva(mode) {
  ...
  if (mode === 'Números calientes') {
    const stats = getNumberStats('primitiva');
    reintegro = Number(stats.hotExtra[0] ?? reintegro);
  }
  if (mode === 'Números fríos') {
    const stats = getNumberStats('primitiva');
    reintegro = Number(stats.coldExtra[0] ?? reintegro);
  }
  if (mode === 'Radar Loto IA') {
    const stats = getNumberStats('primitiva');
    reintegro = Number((stats.hotExtra[0] ?? stats.coldExtra[0] ?? reintegro));
  }
  return { numbers: nums.map(pad2), reintegro: reintegro };
}
