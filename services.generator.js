...
function weightedChoice(values, weights, fallbackMin, fallbackMax) {
  ...
  if (!safe.length) return randomInt(fallbackMin, fallbackMax);

  const total = safe.reduce((acc, item) => acc + item.weight, 0);
...
function pickWeightedHistoricalReintegro(mode) {
  ...
  if (mode === 'Números calientes' && hotPool.length) selected = entries.filter(x => hotPool.includes(x.num));
  if (mode === 'Números fríos' && coldPool.length) selected = entries.filter(x => coldPool.includes(x.num));
...
export function generatePrimitiva(mode) {
  ...
  if (mode === 'Números calientes') reintegro = pickWeightedHistoricalReintegro('Números calientes');
  if (mode === 'Números fríos') reintegro = pickWeightedHistoricalReintegro('Números fríos');
  if (mode === 'Radar Loto IA') reintegro = randomReintegro();

  console.log('[RadarLoto DEBUG] mode=', mode, 'reintegro=', reintegro);
