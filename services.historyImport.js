...
function parsePrimitivaYear(html) {
  ...
  const re = /(\d{2})-(\d{2})-(\d{2})-(\d{2})-(\d{2})-(\d{2})-C(\d{2})\s*-R(\d)\s*\n\s*(?:lun|mar|mie|mié|jue|vie|sab|sáb|dom),\s*(\d{1,2})\s+([a-záéíóú]+)\s+(\d{4})/gi;

  let m;
  while ((m = re.exec(text)) !== null) {
    out.push({
      numbers: [m[1], m[2], m[3], m[4], m[5], m[6]].map(Number),
...
function parseEuroYear(html) {
  ...
  const re = /(\d{2})-(\d{2})-(\d{2})-(\d{2})-(\d{2})-E(\d{2})\s*-E(\d{2})\s*\n\s*(?:lun|mar|mie|mié|jue|vie|sab|sáb|dom),\s*(\d{1,2})\s+([a-záéíóú]+)\s+(\d{4})/gi;

  let m;
  while ((m = re.exec(text)) !== null) {
    out.push({
      numbers: [m[1], m[2], m[3], m[4], m[5]].map(Number),
      stars: [Number(m[6]), Number(m[7])],
      drawDate: toIso(m[8], m[9], m[10])
    });
  }
