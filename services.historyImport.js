...
const YEAR_URL = {
  primitiva: (year) => `https://www.elgordo.com/en/results/primitiva--year-${year}`,
  euromillones: (year) => `https://www.elgordo.com/en/results/euromillions--year-${year}`
};
...
function normalizeMonthName(monthName) {
  return monthName.toLowerCase().replace(/[^a-z]/g, '');
}
...
function parsePrimitivaYear(html) {
  ...
  const dateRe = /^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun),\s*(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})$/i;
