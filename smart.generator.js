...
function buildExtras(game, stats, mode) {
  ...
  if (cfg.extraType === 'reintegro') {
    const fallback = randInt(cfg.extraMin, cfg.extraMax);
    if (mode === 'hot' && Array.isArray(stats?.hotExtra) && stats.hotExtra.length) {
      return { reintegro: safeNumber(stats.hotExtra[0], fallback) };
    }
    if (mode === 'cold' && Array.isArray(stats?.coldExtra) && stats.coldExtra.length) {
      return { reintegro: safeNumber(stats.coldExtra[0], fallback) };
    }
    if (mode === 'radar_ai') {
      return { reintegro: safeNumber(stats?.hotExtra?.[0], safeNumber(stats?.coldExtra?.[0], fallback)) };
    }
    return { reintegro: fallback };
  }

  let pool = rangeArray(cfg.extraMin, cfg.extraMax);
  if (mode === 'hot' && Array.isArray(stats?.hotExtra) && stats.hotExtra.length) {
    pool = [...new Set([...stats.hotExtra, ...pool])];
  }
  if (mode === 'cold' && Array.isArray(stats?.coldExtra) && stats.coldExtra.length) {
    pool = [...new Set([...stats.coldExtra, ...pool])];
  }

  return { stars: uniqueSorted(sampleUnique(pool, cfg.extraCount)) };
}
...
function scoreCombination(game, numbers, stats) {
  ...
  if (sum >= cfg.targetSumMin && sum <= cfg.targetSumMax) score += 4;
  else score -= 4;
...
export function generateSmartNumbers({ game, mode = 'radar_ai', stats = {} }) {
  switch (mode) {
