...
async function importHistory(game) {
  if (game === "powerball") {
    ...
    const rows = json.map((d) => {
      if (!d.winning_numbers || !d.draw_date) return null;
      const parts = String(d.winning_numbers).trim().split(/\s+/).map(Number).filter(Number.isFinite);
      if (parts.length < 6) return null;

      const row = {
        game: "powerball",
        drawDate: String(d.draw_date).slice(0, 10),
        numbers: parts.slice(0, 5).sort((a, b) => a - b),
        stars: [parts[5]],
        multiplier: d.multiplier ? String(d.multiplier) : null,
        jackpot: d.jackpot_amount ? String(d.jackpot_amount) : null,
        cashValue: null
      };
      row.id = ensureDrawId(row);
      return row;
