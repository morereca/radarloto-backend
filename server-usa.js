...
async function importHistory(game) {
  if (game === "powerball") {
    const csvText = await fetchCsv(FEED_URLS.powerball.historyCsv);
    const rows = parsePowerballCsv(csvText).filter((draw) => inHistoryWindow(draw.drawDate));
    mergeHistory(game, rows);
    logSync(`powerball: ${rows.length} sorteos importados`);
    return rows.length;
  }

  const csvText = await fetchCsv(FEED_URLS.megamillions.historyCsv);
  const rows = parseMegaMillionsCsv(csvText).filter((draw) => inHistoryWindow(draw.drawDate));
  mergeHistory(game, rows);
