...
// DEBUG USA: listar combinaciones generadas
app.get("/api/usa/debug/generated", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT id, game, numbers_json, created_at
      FROM usa_generated
    ...
    console.error("DEBUG USA ERROR:", e);
    res.status(500).json({ error: "debug usa failed" });
  }
});
