...
const FEED_URLS = {
  ...
  megamillions: {
    latest: process.env.MEGAMILLIONS_LATEST_URL || "https://www.megamillions.com/winning-numbers/previous-drawings.aspx",
...
const historyState = {
  ...
  sync_log: [],
  source: {
    provider: "official_usa_lottery_pages",
    urls: FEED_URLS
  }
};
...
function parseMegaLatest(html) {
  ...
  // Current public page snippet looks like:
  // "Latest Winning Numbers: Tue., 3/17. 4; 11; 18; 38; 50; 24."
  const m = text.match(/Latest Winning Numbers:\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\.,?\s*(\d{1,2})\/(\d{1,2})\.?\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})(?:\s*\.\s*(\d{1,2})X)?/i);
  if (!m) return null;
...
function parseMegaHistory(html) {
  ...
  // Current rows look like:
  // "3/13/2026. 6; 19; 36; 40; 55; 9. N/A"
  const regex = /(\d{1,2})\/(\d{1,2})\/(\d{4})\.\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})\s*;\s*(\d{1,2})(?:\.\s*([0-9]{1,2}|N\/A)X?)?/g;
  ...
  while ((m = regex.exec(text)) !== null) {
    const row = {
      ...
      stars: [Number(m[9])],
      multiplier: m[10] && m[10] !== "N/A" ? `${m[10]}X` : null,
      jackpot: null,
      cashValue: null
    };
    row.id = ensureDrawId(row);
...
async async function importHistory(game) {
  ...
  const now = new Date();
  const start = new Date(`${currentWindowStartISO()}T00:00:00Z`);

  if (game === "powerball") {
    // Query in smaller windows so the public page exposes all rows instead of only the first batch.
    let allRows = [];
    let cursor = new Date(start);

    while (cursor <= now) {
      const chunkEnd = addDays(cursor, 180);
      const safeEnd = chunkEnd > now ? now : chunkEnd;
      const url = `${FEED_URLS.powerball.history}?gc=powerball&sd=${encodeURIComponent(iso(cursor))}&ed=${encodeURIComponent(iso(safeEnd))}`;
      const html = await fetchText(url);
      const rows = parsePowerballHistory(html);
      allRows = allRows.concat(rows);
      cursor = addDays(safeEnd, 1);
    }

    allRows = dedupeRows(allRows);
    mergeHistory(game, allRows);
    logSync(`powerball: ${allRows.length} sorteos importados`);
    return allRows.length;
  }

  // Mega Millions: query the lower-case previous drawings page in 1-year windows
  // and paginate within each window for full coverage.
  let allRows = [];
  let cursor = new Date(start);
  ...
  while (cursor <= now) {
    ...
    for (let pageNumber = 1; pageNumber <= 20; pageNumber += 1) {
      const url = `${FEED_URLS.megamillions.history}?startDate=${encodeURIComponent(mmddyyyy(cursor))}&endDate=${encodeURIComponent(mmddyyyy(safeEnd))}&pageNumber=${pageNumber}&pageSize=100`;
      const html = await fetchText(url);
      const rows = parseMegaHistory(html);
  ...
  allRows = dedupeRows(allRows);
  mergeHistory(game, allRows);
  logSync(`megamillions: ${allRows.length} sorteos importados`);
  return allRows.length;
}
