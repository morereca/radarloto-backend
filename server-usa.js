...
async function importHistory(game) {
  if (game === "powerball") {
    ...
    const url = `${FEED_URLS.powerball.history}?gc=powerball&sd=${encodeURIComponent(start)}&ed=${encodeURIComponent(end)}`;
  ...
  // Mega Millions paginated history
  const startDate = "01/01/" + (new Date().getUTCFullYear() - YEARS_BACK);
  const endDate = new Date().toLocaleDateString("en-US", { timeZone: "UTC" });
  let allRows = [];
  for (let pageNumber = 1; pageNumber <= 20; pageNumber += 1) {
    const url = `${FEED_URLS.megamillions.history}?endDate=${encodeURIComponent(endDate)}&pageNumber=${pageNumber}&pageSize=100&startDate=${encodeURIComponent(startDate)}`;
    const html = await fetchText(url);
    const rows = parseMegaHistory(html);
    if (!rows.length) break;
    allRows = allRows.concat(rows);
    const oldest = rows[rows.length - 1];
    if (oldest && oldest.drawDate < currentWindowStartISO()) break;
  }
...
apiRouter.get("//history-status", async (_req, res) => {
  await syncHistory(false);
  res.json({
    history: {
      years_back: YEARS_BACK,
...
apiRouter.post("//history/refresh", async (_req, res) => {
...
apiRouter.get("//history-coverage", async (_req, res) => {
  await syncHistory(false);
  res.json({
...
apiRouter.get("//number-stats", async (req, res) => {
...
apiRouter.get("//draws", async (req, res) => {
  const game = normalizeGame(req.query.game);
  if (!["powerball", "megamillions"].includes(game)) {
    return res.status(400).json({ error: "game requerido" });
  }
...
apiRouter.get("//prize-ranking", async (_req, res) => {
  await syncHistory(false);
...
apiRouter.post("//generate-smart", async (req, res) => {
  const game = normalizeGame(req.body?.game);
